...
def get_process_info():
    ...
    try:
        ...
        if info['dispatcher'] and info['dispatcher']['status'] == 'running':
            cursor.execute("INSERT INTO processes (pid, type, status) VALUES (%s, %s, %s)", 
...
def start_dispatcher():
    ...
    try:
        ...
        # Use url_dispatcher_pg.py
        proc = subprocess.Popen(['python', '-u', 'url_dispatcher_pg.py'], 
                              stdout=log_file, 
                              stderr=log_file_err)
...
def start_purger():
    ...
    try:
        ...
        # Use url_purger_pg.py
        proc = subprocess.Popen(['python', '-u', 'url_purger_pg.py'],
                              stdout=log_file,
                              stderr=log_file)
...
def add_fetcher():
    ...
    try:
        ...
        # Use url_fetcher_pg.py
        proc = subprocess.Popen(['python', '-u', 'url_fetcher_pg.py', fetcher_id],
                              stdout=log_file,
                              stderr=log_file)
...
def add_parser():
    ...
    try:
        ...
        # Use url_parser_pg.py
        proc = subprocess.Popen(['python', '-u', 'url_parser_pg.py', parser_id],
                              stdout=log_file,
                              stderr=log_file)
...
def add_indexer():
    ...
    try:
        ...
        # Use abc_indexer_pg.py
        proc = subprocess.Popen(['python', '-u', 'abc_indexer_pg.py', indexer_id],
                              stdout=log_file,
                              stderr=log_file)
...
def get_urls():
    ...
    if status:
        if status == 'new':
            query += ' AND (status = %s OR status IS NULL)'
            params.append('')
        else:
            query += ' AND status = %s'
    ...
    if has_abc == 'true':
        query += ' AND has_abc = TRUE' # Postgres boolean
    ...
    if url_filter:
        ...
        if '%' not in pattern and '_' not in pattern:
            pattern = f'%{pattern}%'
        query += ' AND url ILIKE %s' # ILIKE for case-insensitive
    ...
    rows = cursor.fetchall() # RealDictCursor returns dicts

    # Get total count with same filters
    count_query = 'SELECT COUNT(*) as count FROM urls WHERE 1=1'
    count_params = []

    if status:
        if status == 'new':
            count_query += ' AND (status = %s OR status IS NULL)'
            count_params.append('')
        else:
    ...
    if has_abc == 'true':
        count_query += ' AND has_abc = TRUE'

    if url_filter:
        pattern = url_filter.replace('*', '%')
        if '%' not in pattern and '_' not in pattern:
            pattern = f'%{pattern}%'
        count_query += ' AND url ILIKE %s'
    ...
    if mime_filter:
        ...
        if '%' not in mpat and '_' not in mpat:
            mpat = f'%{mpat}%'
        count_query += ' AND mime_type ILIKE %s'
    ...
    # Normalize rows if they are dicts (from RealDictCursor)
    # The original app manual conversion is redundant if we use RealDictCursor, 
    # but we will keep the explicit dict creation for API consistency and safety.
    urls = []
    for row in rows:
        urls.append({
            'id': row['id'],
            ...
            'link_distance': row['link_distance'] or 0
        })
...
def add_url():
    ...
    try:
        ...
        if ext:
            ...
            cur_check.execute('SELECT 1 FROM refused_extensions WHERE extension = %s', (ext,))
    ...
    try:
        ...
        if host:
            cursor.execute('INSERT INTO urls (url, host) VALUES (%s, %s) ON CONFLICT DO NOTHING', (url, host))
        else:
            cursor.execute('INSERT INTO urls (url) VALUES (%s) ON CONFLICT DO NOTHING', (url,))
        conn.commit()
        
        # Check if inserted or ignored? 
        # cursor.rowcount works for INSERT in psycopg2 usually
...
def get_refused_extensions():
    ...
    rows = cur.fetchall()
    conn.close()
    exts = [{'extension': r['extension'], 'reason': r['reason'], 'created_at': r['created_at']} for r in rows]
...
def add_refused_extension():
    ...
    cur = conn.cursor()
    try:
        cur.execute('INSERT INTO refused_extensions (extension, reason) VALUES (%s, %s) ON CONFLICT DO NOTHING', (ext, reason))
        conn.commit()
        return jsonify({'status': 'ok'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
def delete_refused_extension(extension):
    ...
    cur = conn.cursor()
    try:
        cur.execute('DELETE FROM refused_extensions WHERE extension = %s', (ext,))
        conn.commit()
        return jsonify({'status': 'ok'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
def add_mime_type():
    ...
    try:
        cursor.execute('INSERT INTO mime_types (pattern, enabled) VALUES (%s, %s) ON CONFLICT DO NOTHING',
                      (pattern, enabled))
        conn.commit()
...
def update_mime_type(mime_id):
    ...
    query = f"UPDATE mime_types SET {', '.join(updates)} WHERE id = %s"
...
def delete_mime_type(mime_id):
    ...
    try:
        cursor.execute('DELETE FROM mime_types WHERE id = %s', (mime_id,))
        conn.commit()
...
def get_hosts():
    ...
    if reason:
        ...
        if '%' not in pattern and '_' not in pattern:
            pattern = f'%{pattern}%'
        query += ' AND disabled_reason ILIKE %s'
        params.append(pattern)
...
def update_host(host):
    ...
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute('INSERT INTO hosts (host) VALUES (%s) ON CONFLICT DO NOTHING', (host,))

        if disabled:
            # Keep any existing disabled_reason but record last_access
            cur.execute('UPDATE hosts SET disabled = TRUE, last_access = CURRENT_TIMESTAMP WHERE host = %s', (host,))
            print(f"API: set host {host} disabled via API")
        else:
            # Clearing disabled: remove reason/timestamp
            cur.execute('UPDATE hosts SET disabled = FALSE, disabled_reason = NULL, disabled_at = NULL, last_access = CURRENT_TIMESTAMP WHERE host = %s', (host,))
...
def get_stats():
    ...
    try:
        ...
        # Total size
        cursor.execute('SELECT SUM(size_bytes) as sum FROM urls WHERE size_bytes IS NOT NULL')
        ...
        cursor.execute('''
            ...
            LIMIT 10
        ''')
        stats['top_mime_types'] = {row['mime_type']: row['count'] for row in cursor.fetchall()}
        
        # New metrics
        cursor.execute("SELECT COUNT(*) as count FROM urls WHERE status = 'parsed'")
        stats['total_parsed'] = cursor.fetchone()['count']

        cursor.execute("SELECT COUNT(*) as count FROM urls WHERE has_abc = TRUE")
        stats['total_with_abc'] = cursor.fetchone()['count']

        cursor.execute("SELECT COUNT(*) as count FROM tunebooks")
        stats['total_tunebooks'] = cursor.fetchone()['count']

        cursor.execute("SELECT COUNT(*) as count FROM tunes")
        stats['total_tunes'] = cursor.fetchone()['count']

        cursor.execute("SELECT COUNT(*) as count FROM tunebooks WHERE status = 'indexed'")
        stats['total_indexed_tunebooks'] = cursor.fetchone()['count']

        cursor.execute("SELECT COUNT(*) as count FROM tunes WHERE status = 'parsed' AND intervals IS NULL")
        stats['total_pending_index_tunes'] = cursor.fetchone()['count']

        try:
            cursor.execute("SELECT COUNT(*) as count FROM faiss_mapping")
            stats['faiss_index_size'] = cursor.fetchone()['count']
        except Exception:
        ...
        for row in cursor.fetchall():
            dist = row['link_distance'] if row['link_distance'] is not None else 0
...
if __name__ == '__main__':
    # No init_database() call needed for postgres (managed via scripts)
